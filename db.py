...
def send_conclusion(msg: str, f: bool):
    if f:
...
def db_push(current_data: dict):
    db.child('poems').set(current_data)


def db_add_poem(author: str, title: str, content: str):
...
def db_get_poem(author: str, title: str) -> str:
    if author in data.keys():
        if title.lower().capitalize() in data[author].keys():
...
def db_get_poet(author: str) -> dict:
    if author.lower().capitalize() in data.keys():
        poems = dict()
...
def db_delete_poem(author: str, title: str):
    if author in data.keys():
        if title.lower().capitalize() in data[author].keys():
